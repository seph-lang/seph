 ...
 */
public class PersistentHashMap extends APersistentMap implements EditableCollection, SephObject {
    final int count;
    final INode root;
    ...
    public static PersistentHashMap create(Object... init) {
        ...
        for(int i = 0; i < init.length; i += 2)
            {
                ret = ret.associate(init[i], init[i + 1]);
            }
        ...
        for(; items != null; items = items.next().next())
            {
                ...
                ret = ret.associate(items.first(), RT.second(items));
            }
        return (PersistentHashMap) ret.persistent();
    }
    ...
    public IPersistentMap associate(Object key, Object val) {
        ...
        INode newroot = (root == null ? BitmapIndexedNode.EMPTY : root) 
			.associate(0, Util.hash(key), key, val, addedLeaf);
        if(newroot == root)
            return this;
    ...
    static final class TransientHashMap extends ATransientMap {
        ...
        ITransientMap doAssoc(Object key, Object val) {
            ...
            INode n = (root == null ? BitmapIndexedNode.EMPTY : root)
                .associate(edit, 0, Util.hash(key), key, val, leafFlag);
            if (n != this.root)
                this.root = n; 
    ...
    static interface INode extends Serializable {
        INode associate(int shift, int hash, Object key, Object val, Box addedLeaf);
        ...
        INode associate(AtomicReference<Thread> edit, int shift, int hash, Object key, Object val, Box addedLeaf);
    ...
    final static class ArrayNode implements INode{
        ...
        public INode associate(int shift, int hash, Object key, Object val, Box addedLeaf) {
            int idx = mask(hash, shift);
            INode node = array[idx];
            if(node == null)
                return new ArrayNode(null, count + 1, cloneAndSet(array, idx, BitmapIndexedNode.EMPTY.associate(shift + 5, hash, key, val, addedLeaf)));			
            INode n = node.associate(shift + 5, hash, key, val, addedLeaf);
            if(n == node)
                return this;
        ...
        public INode associate(AtomicReference<Thread> edit, int shift, int hash, Object key, Object val, Box addedLeaf) {
            int idx = mask(hash, shift);
            INode node = array[idx];
            if(node == null) {
                ArrayNode editable = editAndSet(edit, idx, BitmapIndexedNode.EMPTY.associate(edit, shift + 5, hash, key, val, addedLeaf));
                editable.count++;
                return editable;			
            }
            INode n = node.associate(edit, shift + 5, hash, key, val, addedLeaf);
            if(n == node)
                return this;
    ...
    final static class BitmapIndexedNode implements INode{
        ...
        public INode associate(int shift, int hash, Object key, Object val, Box addedLeaf) {
            ...
            if((bitmap & bit) != 0) {
                ...
                if(keyOrNull == null) {
                    INode n = ((INode) valOrNode).associate(shift + 5, hash, key, val, addedLeaf);
                    if(n == valOrNode)
                        return this;
                ...
                if(n >= 16) {
                    ...
                    nodes[jdx] = EMPTY.associate(shift + 5, hash, key, val, addedLeaf);  
                    int j = 0;
                    for(int i = 0; i < 32; i++)
                        if(((bitmap >>> i) & 1) != 0) {
                            if (array[j] == null)
                                ...
                                nodes[i] = EMPTY.associate(shift + 5, Util.hash(array[j]), array[j], array[j+1], addedLeaf);
                            j += 2;
                        }
        ...
        public INode associate(AtomicReference<Thread> edit, int shift, int hash, Object key, Object val, Box addedLeaf) {
            ...
            if((bitmap & bit) != 0) {
                ...
                Object valOrNode = array[2*idx+1];
                if(keyOrNull == null) {
                    INode n = ((INode) valOrNode).associate(edit, shift + 5, hash, key, val, addedLeaf);
                    if(n == valOrNode)
                        return this;
                ...
                if(n >= 16) {
                    INode[] nodes = new INode[32];
                    int jdx = mask(hash, shift);
                    nodes[jdx] = EMPTY.associate(edit, shift + 5, hash, key, val, addedLeaf);  
                    int j = 0;
                    for(int i = 0; i < 32; i++)
                        if(((bitmap >>> i) & 1) != 0) {
                            if (array[j] == null)
                                ...
                                nodes[i] = EMPTY.associate(edit, shift + 5, Util.hash(array[j]), array[j], array[j+1], addedLeaf);
                            j += 2;
                        }
    ...
    final static class HashCollisionNode implements INode{
        ...
        public INode associate(int shift, int hash, Object key, Object val, Box addedLeaf) {
            ...
            return new BitmapIndexedNode(null, bitpos(this.hash, shift), new Object[] {null, this})
                .associate(shift, hash, key, val, addedLeaf);
        }
        ...
        public INode associate(AtomicReference<Thread> edit, int shift, int hash, Object key, Object val, Box addedLeaf) {
            ...
            return new BitmapIndexedNode(edit, bitpos(this.hash, shift), new Object[] {null, this, null, null})
                .associate(edit, shift, hash, key, val, addedLeaf);
        }	
    ...
    private static INode createNode(int shift, Object key1, Object val1, int key2hash, Object key2, Object val2) {
        ...
        return BitmapIndexedNode.EMPTY
            .associate(edit, shift, key1hash, key1, val1, _)
            .associate(edit, shift, key2hash, key2, val2, _);
    }
